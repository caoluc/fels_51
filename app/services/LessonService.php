...
class LessonService extends BaseService
{
    ...
    public static function getActivities($userId)
    {
        ...
        $users->load('follow');
        $userActivity = Activity::where('user_id', '=', $userId)->first();
        if (!$userActivity) {
            return;
        }
        $follow = UserFollow::where('user_id', '=', $userId)->first();
        if (!$follow) {
        ...
        foreach ($users as $user) {
            $activities = Activity::where('user_id', '=', $userId)->orWhere('user_id', '=', $users->follow()->first()->id)->get();
        }
